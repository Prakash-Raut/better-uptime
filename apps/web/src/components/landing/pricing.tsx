	...
	return (
		<section ref={ref} className="w-full border-b dark:bg-neutral-950">
			<div className="container mx-auto flex max-w-6xl flex-col items-center space-y-10 border-x px-8 py-5">
				<h3
					className={cn(
						"font-semibold text-2xl text-green-400 uppercase tracking-tight transition-all duration-600",
				...
				<h2
					className={cn(
						"text-center font-bold text-3xl tracking-tight transition-all duration-700 md:text-5xl",
						isVisible
					...
					<div className="flex flex-col items-center justify-center">
						...
						>
							<TabsList className="h-11 rounded-full border bg-background">
								<TabsTrigger
									value="monthly"
									className="rounded-full px-4 transition-all duration-200 data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
								>
									Monthly
								</TabsTrigger>
								<TabsTrigger
...
function PricingCard({
	...
	return (
		<div
			ref={ref}
			className={cn(
				"relative border bg-background p-6 px-8 transition-all duration-500 hover:scale-[1.02] hover:shadow-lg",
				{
		...
		>
			...
			<Button
				...
				size="lg"
				className="mt-6 w-full rounded-full text-base transition-all duration-200 hover:scale-105"
			>
			...
			<ul className="space-y-3">
				{plan.features.map((feature) => (
					<li
						key={feature.title}
						className="flex items-start gap-1.5 transition-colors duration-200 hover:text-primary"
					>
						...
						{feature.tooltip && (
							<Tooltip>
								<TooltipTrigger className="cursor-help">
									<CircleHelpIcon className="mt-1 h-4 w-4 text-gray-500 transition-colors duration-200 hover:text-gray-700" />
								</TooltipTrigger>
